...
function CreateArtifactDialog({ onClose }: { onClose: () => void }) {
  ...
  const mutation = useMutation({
    mutationFn: () => apiClient.createArtifact(type, topic),
    onSuccess: data => {
...
function PreviewDialog({ filename, onClose }: { filename: string; onClose: () => void }) {
  ...
  useEffect(() => {
    ...
    fetch(apiClient.downloadArtifactUrl(filename))
      .then(r => r.text())
      .then(text => { setContent(text); setLoading(false) })
      .catch(() => { setContent('Erro ao carregar arquivo.'); setLoading(false) })
  }, [filename])
        ...
        <div className="flex items-center justify-between border-b border-zinc-800 px-6 py-4">
          ...
          <div className="flex gap-2">
            <Button variant="outline" size="sm" asChild>
              <a href={apiClient.downloadArtifactUrl(filename)} download={filename}>
            ...
            {isMarkdownArtifact(filename) && (
              <Button variant="outline" size="sm" asChild>
                <a href={apiClient.downloadArtifactPdfUrl(filename)} download={toPdfName(filename)}>
      ...
      {artifacts && artifacts.length > 0 && (
        <div className="space-y-2">
          {artifacts.map(artifact => {
            ...
            return (
              <Card key={artifact.filename} className="hover:border-zinc-700 transition-colors">
                <CardContent className="flex items-center justify-between py-4">
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="min-w-0">
                      ...
                      <p className="text-xs text-zinc-500">
                        {formatBytes(artifact.size)} · {formatDate(artifact.created_at)}
                      </p>
                    </div>
                  </div>
                  <div className="flex items-center gap-2 shrink-0 ml-4">
                    ...
                    <Button variant="outline" size="sm" asChild>
                      <a
                        href={apiClient.downloadArtifactUrl(artifact.filename)}
                    ...
                    {isMarkdown && (
                      <Button variant="outline" size="sm" asChild>
                        <a
                          href={apiClient.downloadArtifactPdfUrl(artifact.filename)}
                          download={toPdfName(artifact.filename)}
                        >
                          <FileText className="h-4 w-4 mr-1" />
                          PDF
                        </a>
